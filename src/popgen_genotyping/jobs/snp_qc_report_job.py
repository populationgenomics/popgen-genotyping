"""Hail Batch chain that produces the per-SNP QC side-car exclusion list.
...
1. ``bcftools_image`` extract — pulls ``GenTrain_Score`` and ``Cluster_Sep`` (plus
   ...
   header-less TSV.
2. ``driver_image`` synthesis — runs ``scripts/snp_qc_report.py``, joining the
...
def run_snp_qc_report(
    *,
    ...
    """Queue the EGT-extract + synthesis chain.

    Args:
        egt_info_bcf_path: Cloud path to the references-repo EGT info BCF.
        egt_info_bcf_index_path: Cloud path to its ``.csi`` index.
        merged_vmiss_path: Cloud path to the merged-set ``.vmiss`` from ``Plink2Qc``.
        ...
        output_audit_tsv_path: Cloud destination for the bgzipped audit TSV.
        output_exclusion_list_path: Cloud destination for the exclusion ``.snplist``.
        output_summary_tsv_path: Cloud destination for the summary TSV.
